...
{
    public abstract class testingWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static testingWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(testingWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<testingDbContext> action)
        {
            using (var context = IocManager.Resolve<testingDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<testingDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<testingDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<testingDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<testingDbContext, Task<T>> func)
        {
            T result;
